...
function Stop-WithError {
    ...
    Write-Host "  [X] ERROR OCCURRED!                                      " -ForegroundColor Red
    Write-Host "  ========================================================" -ForegroundColor Red
    Write-Host ""
...
Write-Host ""
Write-Host "  PHANTOMS API" -ForegroundColor Magenta
...
# Brief pause to let dotnet fully release file handles
...
Write-Info "Compressing files with ZipFile..."

try {
    Add-Type -AssemblyName System.IO.Compression.FileSystem
    if (Test-Path $PUBLISH_ZIP) { Remove-Item $PUBLISH_ZIP -Force }
    [System.IO.Compression.ZipFile]::CreateFromDirectory($PUBLISH_DIR, $PUBLISH_ZIP)
} catch {
...
$accountUser = if ($azAccount -and $azAccount.user -and $azAccount.user.name) { $azAccount.user.name } else { "N/A" }
$accountName = if ($azAccount -and $azAccount.name) { $azAccount.name } else { "N/A" }
Write-Info "Azure Account : $accountUser"
Write-Info "Subscription  : $accountName"
Write-Host ""
