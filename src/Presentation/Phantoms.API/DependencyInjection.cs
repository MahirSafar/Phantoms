...
using Phantoms.API.Middleware;
using Phantoms.API.Services;
using Phantoms.Application.Common.Interfaces;
using Phantoms.Domain.Constants;

namespace Phantoms.API;
...
public static class DependencyInjection
{
    public static IServiceCollection AddPresentation(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        {
            // Return a clean JSON response matching Result wrapper on 429
            ...
            // Named policy: "fixed" – 60 requests per minute per IP (general endpoints)
            ...
            // Named policy: "auth" – 20 requests per minute per IP (login/register)
        ...
        .AddJwtBearer(options =>
        {
            ...
            options.Events = new JwtBearerEvents
            {
                OnChallenge = async context =>
                {
                    ...
                    var response = new { succeeded = false, message = "Authentication is required.", data = (object?)null, errors = new[] { "You must provide a valid Bearer token." } };
                ...
                OnForbidden = async context =>
                {
                    ...
                    context.Response.ContentType = "application/json";
                    var response = new { succeeded = false, message = "You do not have permission to access this resource.", data = (object?)null, errors = new[] { "Access denied." } };
                    await context.Response.WriteAsJsonAsync(response);
                }
            };
        })
        ...
        services.AddSwaggerGen(options =>
        {
            options.SwaggerDoc("v1", new OpenApiInfo
            {
                Title = "Phantoms API",
                Version = "v1",
                Description = "Phantoms Onion Architecture API"
            });
